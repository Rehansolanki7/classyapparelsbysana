...
export default function ShopClient({ products, categories, settings, initialQuery, initialCategorySlug, initialBagOpen = false }: { products: CatalogProduct[]; categories: ManagedCategory[]; settings: StorefrontSettings; initialQuery: string; initialCategorySlug: string; initialBagOpen?: boolean }) {
  const router = useRouter();
  const [query, setQuery] = useState(initialQuery);
  const [category, setCategory] = useState(() => categories.some((item) => item.slug === initialCategorySlug) ? initialCategorySlug : "all");
  ...
  const availableCategories = useMemo(() => categories.filter((category) => products.some((product) => product.categoryId === category.id)), [categories, products]);
  const visible = useMemo(() => products.filter((product) => {
    ...
    return matchesQuery && matchesCategory && matchesColour && matchesAvailability && matchesPrice;
  }), [products, query, category, colour, availability, priceLimit]);
  const selectedCategory = availableCategories.find((item) => item.slug === category);
  const whatsappMessage = `Hi Sana, I’m browsing the Classy Apparels shop${selectedCategory ? ` in ${selectedCategory.name}` : ""}${query.trim() ? ` and looking for ${query.trim()}` : ""}${colour !== "all" ? ` in ${colour}` : ""}. Could you help me choose a piece?`;

  function chooseCategory(slug: string) {
    setCategory(slug);
    const params = new URLSearchParams(window.location.search);
    if (slug === "all") params.delete("category"); else params.set("category", slug);
    const href = `/shop${params.size ? `?${params.toString()}` : ""}`;
  ...
  function addToBag(product: CatalogProduct) {
    const size = product.hasSizes ? selectedSizes[product.id] ?? "" : product.variants.find((item) => item.active && item.stock > 0)?.size ?? "";
  ...
  return (
    <main className="shop-page">
      ...
      {availableCategories.length > 0 && <nav className="shop-category-filter" aria-label="Shop by category"><span>Shop by style</span><div><button type="button" className={category === "all" ? "active" : ""} aria-pressed={category === "all"} onClick={() => chooseCategory("all")}>All pieces</button>{availableCategories.map((item) => <button type="button" key={item.id} className={category === item.slug ? "active" : ""} aria-pressed={category === item.slug} onClick={() => chooseCategory(item.slug)}>{item.name}</button>)}</div></nav>}
      ...
      {visible.length ? <section className="shop-grid">{visible.map((product) => {
        ...
        const canBuy = product.hasSizes ? sizes.length > 0 : product.variants.some((variant) => variant.active && variant.stock > 0);
        return <article className="shop-card" key={product.id}><a href={`/products/${product.slug}`} className="shop-card-image"><img src={product.images[0]} alt={product.name} loading="lazy" decoding="async" />{stock <= 3 && <span>{stock === 0 ? "Sold out" : `Only ${stock} left`}</span>}</a><div><p>{[product.category, product.color].filter(Boolean).join(" · ") || "Sana edit"}</p><h2><a href={`/products/${product.slug}`}>{product.name}</a></h2><div className="shop-price"><strong>{money(product.price)}</strong>{product.compareAt > product.price && <del>{money(product.compareAt)}</del>}</div><p className="card-sizes">{product.hasSizes ? (sizes.map((variant) => variant.size).join(" · ") || "Restocking soon") : "Free-size · no size needed"}</p><div className="shop-quick-add">{product.hasSizes ? <label><span className="sr-only">Choose size for {product.name}</span><SizeSelect value={selectedSizes[product.id] ?? ""} options={sizes} onChange={(size) => { setSelectedSizes((current) => ({ ...current, [product.id]: size })); setNoticeId(""); }} label={`Choose size for ${product.name}`} disabled={!sizes.length || !hydrated} /></label> : <span className="no-size-quick-add">No size needed</span>}<button onClick={() => addToBag(product)} disabled={!canBuy || !hydrated}>Add to bag</button></div>{noticeId === product.id && <p className="quick-add-notice" role="alert">{product.hasSizes ? "Choose a size first." : "This piece is sold out."}</p>}</div></article>;
      })}</section> : <div className="shop-empty"><h2>{selectedCategory ? `No pieces in ${selectedCategory.name} yet.` : "No pieces match that search."}</h2><button className="text-link" onClick={() => { setQuery(""); if (selectedCategory) chooseCategory("all"); }}>{selectedCategory ? "Browse all pieces" : "Clear search"}</button></div>}
