...
export default function ShopClient({ products, initialQuery }: { products: CatalogProduct[]; initialQuery: string }) {
  ...
  return (
    <main className="shop-page">
      <div className="shop-announcement">Complimentary shipping on prepaid orders over ₹1,499</div>
      <header className="shop-header"><Link href="/" className="checkout-back">← Home</Link><Link className="checkout-wordmark" href="/"><span>Classy Apparels</span></Link><div className="shop-header-actions"><Link href="/wishlist">Wishlist</Link><Link href="/account">My orders</Link><Link href="/track-order">Track order</Link><a href="https://wa.me/917715910151" target="_blank" rel="noreferrer">Need help?</a><button className="shop-bag-button" onClick={() => setBagOpen(true)} aria-label={`Open bag with ${bagCount} items`}><BagIcon />{bagCount > 0 && <span>{bagCount}</span>}</button></div></header>
      ...
      <section className="shop-help"><div><p className="kicker">Not sure about fit?</p><h2>Sana can help you choose.</h2></div><p>Send your usual size or measurements on WhatsApp. We’ll compare them with the garment before you order.</p><a className="button button-dark" href={`https://wa.me/917715910151?text=${encodeURIComponent("Hi Sana, I need help choosing a size from the website.")}`} target="_blank" rel="noreferrer">Ask on WhatsApp</a></section>
      ...
      <div className={`overlay ${bagOpen ? "show" : ""}`} onClick={() => setBagOpen(false)} />
      <aside className={`side-panel cart-panel shop-cart-panel ${bagOpen ? "open" : ""}`} aria-hidden={!bagOpen}>
        <div className="panel-header"><span>Your bag · {bagCount}</span><button className="icon-button" onClick={() => setBagOpen(false)} aria-label="Close bag">×</button></div>
        {!bagLines.length ? <div className="empty-cart"><BagIcon /><h2>Your bag is waiting</h2><p>Choose a size and add a piece from the Sana edit.</p><button className="button button-dark" onClick={() => setBagOpen(false)}>Continue shopping</button></div> : <div className="cart-content"><div className="shop-bag-items">{bagLines.map(({ item, product, index }) => <div className="shop-bag-item" key={`${item.productId}-${item.size}`}><img src={product.images[0]} alt="" /><div><strong>{product.name}</strong><span>Size {item.size}</span><small>{money(product.price * item.quantity)}</small><div className="quantity-control"><button onClick={() => changeQuantity(index, item.quantity - 1)} aria-label="Decrease quantity">−</button><span>{item.quantity}</span><button onClick={() => changeQuantity(index, item.quantity + 1)} aria-label="Increase quantity">+</button></div></div><button className="bag-remove" onClick={() => changeQuantity(index, 0)} aria-label={`Remove ${product.name}`}>×</button></div>)}</div><div className="shipping-meter"><div><span>{bagSubtotal >= 1499 ? "You’re getting complimentary shipping" : `${money(1499 - bagSubtotal)} away from free shipping`}</span><strong>{bagSubtotal >= 1499 ? "✓" : ""}</strong></div><div className="meter-track"><div className="meter-fill" style={{ width: `${Math.min(100, (bagSubtotal / 1499) * 100)}%` }} /></div></div><div className="cart-summary"><div><span>Subtotal</span><strong>{money(bagSubtotal)}</strong></div><small>Availability and the final total are confirmed securely at checkout.</small></div><a href={checkoutHref} className="button button-dark checkout-button">Secure checkout <span>→</span></a><p className="secure-line">Protected checkout · UPI · Cards</p></div>}
      </aside>
    </main>
  );
}
