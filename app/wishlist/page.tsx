...
export default async function WishlistPage() {
  ...
  return <main className="wishlist-page"><header className="account-header"><Link href="/shop">← Shop</Link><Link className="checkout-wordmark" href="/"><span>Classy Apparels</span></Link><Link href="/account">My account</Link></header><section><p className="kicker">Your favourites</p><h1>Wishlist</h1>{products.length ? <div className="wishlist-grid">{products.map((product) => <article key={product.id}><img src={product.images[0]} alt={product.name} /><div><p>{product.color}</p><h2>{product.name}</h2><Link className="button button-dark" href={`/products/${product.slug}`}>View piece</Link></div></article>)}</div> : <div className="account-empty"><h3>Your wishlist is empty.</h3><p>Save pieces you want to return to later.</p><Link className="button button-dark" href="/shop">Browse the shop</Link></div>}</section></main>;
}
