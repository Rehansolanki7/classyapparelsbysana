...
export default async function AccountPage() {
  ...
  try {
    const db = getDb();
    const rows = await db.select().from(orders).where(eq(orders.email, user.email)).orderBy(desc(orders.createdAt)).limit(100);
