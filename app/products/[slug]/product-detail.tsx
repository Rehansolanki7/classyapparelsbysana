...
export default function ProductDetail({ product }: { product: CatalogProduct }) {
  ...
  const availableVariants = product.variants.filter((variant) => variant.active && variant.stock > 0);
  const defaultVariant = product.variants.find((variant) => variant.active) ?? product.variants[0];
  const effectiveSize = product.hasSizes ? size : defaultVariant?.size ?? "";
  const selectedStock = product.variants.find((variant) => variant.size === effectiveSize)?.stock ?? 0;
  ...
  const whatsappMessage = `Hi Sana, I’m interested in the ${product.name}${product.color ? ` in ${product.color}` : ""}${product.hasSizes && size ? `, size ${size}` : ""}${quantity > 1 ? `, quantity ${quantity}` : ""}. Could you help me with sizing and availability?`;
  useEffect(() => {
    let active = true;
    fetch("/api/wishlist")
  ...
  async function subscribeRestock(event: React.FormEvent) {
    ...
    setRestockBusy(true);
    setRestockNotice("");
    const variantId = product.variants.find((variant) => variant.size === effectiveSize)?.id ?? null;
    try {
      ...
      <div className="product-page-grid">
        ...
        <section className="product-page-copy">
          ...
          {product.hasSizes ? <>
            ...
            <div className="size-options">{availableVariants.map((variant) => <button type="button" key={variant.id} className={size === variant.size ? "selected" : ""} onClick={() => { setSize(variant.size); setQuantity((current) => Math.max(1, Math.min(current, variant.stock))); }} aria-pressed={size === variant.size}>{variant.size}</button>)}</div>
            {!availableVariants.length && <p className="stock-note">No sizes currently available.</p>}
            {size && selectedStock <= 3 && selectedStock > 0 && <p className="stock-note">Only {selectedStock} left in {size}</p>}
          </> : <div className="no-size-choice"><strong>No size needed</strong><span>Free-size / unstitched piece</span></div>}
          {effectiveSize && selectedStock <= 0 && <form className="restock-form" onSubmit={subscribeRestock}><label><span>{product.hasSizes ? "Size sold out" : "Currently sold out"} — email me when it returns</span><input type="email" value={restockEmail} onChange={(event) => setRestockEmail(event.target.value)} placeholder="you@example.com" required disabled={restockBusy} /></label><button type="submit" disabled={restockBusy}>{restockBusy ? "Saving…" : "Notify me"}</button>{restockNotice && <small role="status">{restockNotice}</small>}</form>}
          <label className="product-quantity">Quantity<select value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} disabled={!effectiveSize || selectedStock <= 0}>{Array.from({ length: Math.max(1, Math.min(5, selectedStock)) }, (_, index) => index + 1).map((value) => <option key={value}>{value}</option>)}</select></label>
          {effectiveSize && selectedStock > 0 ? <Link className="button button-dark product-buy" href={`/checkout?product=${encodeURIComponent(product.id)}&size=${encodeURIComponent(effectiveSize)}&qty=${quantity}`}>Buy securely · {money(product.price * quantity)}</Link> : <button type="button" className="button button-dark product-buy" disabled aria-describedby="size-selection-help">{product.hasSizes ? (size ? "This size is sold out" : "Choose a size to buy securely") : "This piece is sold out"}</button>}
          {product.hasSizes && !size && <p className="stock-note" id="size-selection-help">Choose your preferred size before checkout.</p>}
