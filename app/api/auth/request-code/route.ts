...
export async function POST(request: Request) {
  ...
  if (!rate.allowed) {
    await recordEvent({ severity: "security", eventType: "auth.email_code_rate_limited", detail: "IP rate limit" });
  ...
  try {
    ...
    await recordEvent({ severity: "security", eventType: "auth.email_code_request_failed", detail: "Invalid request or mail delivery failure" });
