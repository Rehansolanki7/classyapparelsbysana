...
export async function sendPaidOrderNotifications(orderId: string, force = false) {
  ...
  const customerNote = order.customerNote ? `<p style="padding:12px 14px;border-left:3px solid #d4a24c;background:#f8edda"><strong>Note for Sana</strong><br>${escapeHtml(order.customerNote).replace(/\n/g, "<br>")}</p>` : "";
  const siteUrl = (process.env.SITE_URL ?? "https://classyapparelsbysana.com").replace(/\/$/, "");
  const adminHtml = `<div style="font-family:Arial,sans-serif;color:#223133;max-width:640px"><h1 style="font-family:Georgia,serif;font-weight:400">New paid order ${escapeHtml(order.orderNumber)}</h1><p><strong>${escapeHtml(order.customerName)}</strong><br>${escapeHtml(order.phone)} · ${escapeHtml(order.email)}</p><p>${address}</p>${customerNote}<table style="width:100%;border-collapse:collapse;border-top:1px solid #d8dddd;border-bottom:1px solid #d8dddd">${itemRows}</table><p style="font-size:20px"><strong>Total: ${escapeHtml(rupees(order.totalPaise))}</strong></p><p><a href="${siteUrl}/admin" style="color:#5d9798">Open the Admin Studio</a></p></div>`;
